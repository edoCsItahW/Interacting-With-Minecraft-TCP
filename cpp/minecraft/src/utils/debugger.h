...
namespace minecraft {
    ...
    template<typename F>
    struct FuncType : FuncType<decltype(&F::operator())> {};
    ...
    public:
         ...
         * */
        explicit Debugger(F&& func, bool exit = false, const std::source_location& loc = std::source_location::current());

        explicit Debugger(F&& func, const std::function<void(std::exception)>& callback, bool exit = false, const std::source_location& loc = std::source_location::current());
    ...
    class Debugger<F C::*> {
    public:
        explicit Debugger(F C::* func, bool exit = false, const std::source_location& loc = std::source_location::current());
