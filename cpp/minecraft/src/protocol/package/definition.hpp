...
namespace minecraft::protocol {
    ...
    namespace detail {
        template<typename F>
        void parseKnownPacket(const State state, const std::vector<std::byte>& data, const F& f) {
            ...
            try {
                switch (state) {
                    using enum State;
                    case HANDSHAKE: parseHandshakePacket<F>(packetID, data, f);
                    case STATUS: parseStatusPacket<F>(packetID, data, f);
                    case LOGIN: parseLoginPacket<F>(packetID, data, f);
                    case PLAY: parsePlayPacket<F>(packetID, data, f);
                }
        ...
        template<typename F>
        void parsePlayPacket(const int id, const std::vector<std::byte>& data, const F& f) {
    ...
    template<typename F>
    void parsePacket(const State state, const std::vector<std::byte>& data, const F& f) {
        try {
            detail::parseKnownPacket<F>(state, data, f);
        } catch (const std::exception&) { detail::parseUnknownPacket<F>(data, f); }
    }
